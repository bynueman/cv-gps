...
- `src/lib/content.ts` — brands, 14 products (`kuicipProducts`, `putriTekoProducts`), 6 articles, `company` info. Adding a product/article object here automatically propagates to catalogs, static params, related/featured logic. Putri Teko products need `group: "rtd" | "brew"`. Some SKUs and all `company` contact details are flagged as placeholders (`placeholder: true` / README handoff checklist) — don't treat them as confirmed facts, and don't add factual claims (certifications, export volumes, dates) to copy; it is deliberately inquiry-oriented.
