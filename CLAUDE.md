...
- `src/lib/content.ts` — brands, 20 products (8 `kuicipProducts`, 12 `putriTekoProducts`), 6 articles, `company` info, plus `logos`/`groupShots` asset maps. Adding a product/article object here automatically propagates to catalogs, static params, related/featured logic. Putri Teko products need `group: "rtd" | "brew"` and `packaging: "botol" | "kotak" | "toples" | "kemasan"` (drives catalog grouping, card aspect ratio, and the packaging badge). All products reference real studio photos in `public/images/**`; `company` contact details are still placeholders — don't treat them as confirmed facts, and don't add factual claims (certifications, export volumes, dates) to copy; it is deliberately inquiry-oriented.
